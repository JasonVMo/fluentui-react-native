...
export function resolveModuleName(
  ...
  const optionsWithSuffixes = isPackageRef(moduleName) ? options : { ...options, moduleSuffixes: context.platformFileExtensions };
  ...
  //  default.
  //
  const resolutionMode: ts.ModuleKind.CommonJS | ts.ModuleKind.ESNext | undefined = undefined;

  return resolveModuleName(moduleName, containingFile, optionsWithSuffixes, context.host, cache, redirectedReference, resolutionMode);
}
...
export function resolveModuleNameLiterals(
  ...
  for (const moduleLiteral of moduleLiterals) {
    ...
    const resolvedModule = resolveModuleName(context, finalModuleName, containingFile, redirectedReference, options, typescript);
 ...
 */
export function resolveTypeReferenceDirectiveReferences<T extends ts.FileReference | string>(
  ...
  for (const ref of typeDirectiveReferences) {
    ...
    //  And we should store each per-directory cache in a map, attached to `context`.
    //
    const cache: ts.TypeReferenceDirectiveResolutionCache | undefined = undefined;
...
export function resolveTypeReferenceDirectives(
  ...
  for (const typeDirectiveName of typeDirectiveNames) {
    ...
    const { resolvedTypeReferenceDirective: directive } = resolveTypeReferenceDirective(
      name,
      containingFile,
      optionsWithSuffixes,
      host,
